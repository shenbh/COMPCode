...
import com.newland.comp.adapter.my.MyAttendanceLeaveAdapter;
 ...
 * 
 */
public class MyAttendanceLeaveActivity extends BaseActivity implements IXListViewListener {
	private PullRefreshListView pullRefreshListView;
	private int page_index = 1;// 当前页码
	// private int page_row = 4;// 每页显示条数
	private String quarter = "2015-09";// 查询季度

	private List<LeaveCountData> list;
	private MyAttendanceLeaveAdapter myAttendanceLeaveAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.my_attendance_leave_detail);
		intent = getIntent();
	...
	private void setTitle() {
		...
		if (center_tv != null)
			center_tv.setText("请假明细");
		if (left_btn != null)
			left_btn.setVisibility(View.VISIBLE);
		left_btn.setOnClickListener(new OnClickListener() {
			
			public void onClick(View v) {
				MyAttendanceLeaveActivity.this.finish();
			}
		});
		if (right_btn != null) {
	...
	private void getDataFromServer(int index) {
		...
		fh.post(HttpUtils.URL, params, new AjaxCallBack<Object>() {
			...
			public void onFailure(Throwable t, int errorNo, String strMsg) {if(StringUtil.isNotEmpty(strMsg))strMsg=replaceErroStr(strMsg);
				...
				if (MyAttendanceLeaveActivity.this==null) {
					return;
				}
			...
			public void onSuccess(Object t) {
				...
				if (jsonInfov2 != null) {
					if (JsonInfoV2.SUCCESS_CODE.equals(jsonInfov2.getResultCode())) {// 正常返回数据
						...
						List<LeaveCountData> mlist = jsonInfo.getDataList(LeaveCountData.class);
						if (page_index == 1) {// 刷新
							if (mlist != null && mlist.size() > 0) {
								...
								list = mlist;
								myAttendanceLeaveAdapter = new MyAttendanceLeaveAdapter(MyAttendanceLeaveActivity.this, list);
								pullRefreshListView.setAdapter(myAttendanceLeaveAdapter);
							...
							if (mlist != null && mlist.size() > 0) {
								list.addAll(mlist);
								myAttendanceLeaveAdapter.notifyDataSetChanged();
							} else {
