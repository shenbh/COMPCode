 ...
 *
 */
public class MarkingWorkloadFragment extends BaseFragment {
	Context context;
	private View mView;
	private ListView listView;
	String month;
	private MarkingWorkloadAdapter adapter;
	...
	public void refresh() {
		...
		params.put("method", "payroll_marking");
			...
			public void onSuccess(Object t) {
				...
				if (jsonInfov2 != null) {
					...
					if (JsonInfoV2.SUCCESS_CODE.equals(jsonInfov2.getResultCode())) // 正常返回数据
					{
						...
						// 服务器传回来的用户信息
						List<Marking> list = jsonInfo.getDataList(Marking.class);
						if (list.size()>0) {
							adapter = new MarkingWorkloadAdapter(context, list, getActivity().findViewById(R.id.mColumnHorizontalScrollView));
	...
	public MarkingWorkloadFragment(Context context) {
		super();
		this.context = context;
	}

	public static MarkingWorkloadFragment newInstance(Context context) {
		return new MarkingWorkloadFragment(context);
	}
}
