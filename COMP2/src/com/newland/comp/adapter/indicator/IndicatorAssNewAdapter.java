 ...
 *
 */
public class IndicatorAssNewAdapter extends BaseAdapter {

	private Context context;
	private List<IndicatorAssResultDataRelation> list = new ArrayList<IndicatorAssResultDataRelation>();
	private LayoutInflater mInflater;

	public IndicatorAssNewAdapter(Context context, List<IndicatorAssResultDataRelation> list) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.indicator_acc_row, null);// 一行的样式
		...
		IndicatorAssResultDataRelation bean = list.get(position * 2);
		holder.left_key.setText(StringUtil.noNull(bean.zb_name));
		holder.left_value.setText(StringUtil.noNull(bean.value));
		...
		if (position == this.getCount() - 1 && list.size() % 2 == 1) {
			...
			IndicatorAssResultDataRelation bean2 = list.get(position * 2 + 1);
			holder.right_key.setText(StringUtil.noNull(bean2.zb_name));
			holder.right_value.setText(StringUtil.noNull(bean2.value));
		}
